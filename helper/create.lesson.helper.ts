...
export const validTitleLesson = (titleLesson: ErrorResponse, setTitleLesson: React.Dispatch<SetStateAction<ErrorResponse>>): boolean => {
    if (titleLesson.value.trim() === '') {
        setTitleLesson({
            ...titleLesson,
            error: true,
            message: 'Tiêu đề bài giảng không được để rỗng!'
        })
        return false;
    }
    return true;
