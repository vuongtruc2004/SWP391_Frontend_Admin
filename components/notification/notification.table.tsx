  ...
  const columns: TableProps<NotificationResponse>["columns"] = [
    {
      ...
      render: (text, record, index) => (
        <>{index + 1 + (page - 1) * notificationPageResponse.pageSize}</>
      ),
    },
    {
      title: "Tiêu đề",
    ...
    },
    {
      ...
      render: (_, record: NotificationResponse) =>
        record.setDate !== null
          ? dayjs(record.setDate).format("DD/MM/YYYY HH:MM")
    ...
    },
    {
      ...
      render: (status: string) => {
        if (status === "PENDING") {
          return <p className="text-orange-500">Đang chờ gửi</p>;
        } else {
          return <p className="text-green-500">Đã gửi</p>;
        }
      },
    },
    {
      ...
      render: (_, record: NotificationResponse) => (
        <Space size="middle">
          ...
          {session?.user.roleName === "ADMIN" && (
            <>
              <Tooltip
                key={`delete-${record.notificationId}`}
                placement="bottom"
                title={"Xóa thông báo"}>
                <Popconfirm
                  placement="left"
                  ...
                  cancelText="Không"
                  onConfirm={() => {
                    handleDelete(record.notificationId);
                  }}>
