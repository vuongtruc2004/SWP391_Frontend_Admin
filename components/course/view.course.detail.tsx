...
export const countTotalTime = (course: CourseDetailsResponse): string => {
  ...
  for (let chapter of course.chapters) {
    totalSeconds += chapter.lessons.reduce(
      (sum, lesson) => sum + Math.max(lesson.duration || 0, 60),
      0
    );
  }
...
const ViewCourseDetail = (props: { courseDetail: CourseDetailsResponse }) => {
      ...
      <div className="bg-slate-600 w-full h-[50%]">
        <div className="ml-28 my-14">
          ...
          <Rate
            ...
            defaultValue={
              courseDetail.averageRating === null
                ? 0
                : Number(courseDetail.averageRating.toFixed(1))
            }
            style={{ marginTop: "25px" }}
          />{" "}
          <span className="ml-4 text-[20px] text-white font-semibold">
            {courseDetail.averageRating === null
              ? 0
              : courseDetail.averageRating.toFixed(1)}
          </span>
          <div className="text-white font-semibold text-[16px]">
            ({courseDetail.totalRating === null ? 0 : courseDetail.totalRating}{" "}
            đánh giá)
          </div>
          <div className="mt-8">
            {courseDetail.subjects.map((subject) => (
              <Tag color="#2db7f5" key={subject.subjectId}>
                {subject.subjectName}
              </Tag>
            ))}
          </div>
          <div className="mt-5 flex gap-3">
            <Avatar
              size={64}
              src={`${storageUrl}/avatar/${courseDetail.expert.user.avatar}`}
            />
            <div className="self-center">
              <h2 className="text-white font-semibold text-[20px]">
                {courseDetail.expert.user.fullname}
              </h2>
              <p className="text-[#dee2e6]">{courseDetail.expert.job}</p>
            </div>
          </div>
        </div>
      </div>
      <div className="grid grid-cols-[60%_30%] ">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-x-5 gap-y-3 mt-3">
              {courseDetail.objectives.map((object, index) => (
                <p key={`${courseDetail.courseId}-objective-${index}`}>
                  <CheckOutlined
                    style={{ color: "#52b788", fontSize: "20px" }}
                  />{" "}
                  {object}
                </p>
              ))}
          ...
          <div className="mt-5">
            ...
            <div className="flex items-center justify-between text-gray-600 mb-4 px-1 text-sm">
              <p className="flex items-center gap-x-1">
                ...
                <span>•</span>
                <strong className="text-black">
                  {getNumberOfLessonType(courseDetail, "VIDEO")}
                </strong>{" "}
                bài giảng
                <span>•</span>
                <strong className="text-black">
                  {getNumberOfLessonType(courseDetail, "DOCUMENT")}
                </strong>{" "}
                bài đọc
                <span>•</span>
        ...
        <div className="border bg-white rounded-lg shadow-[0_0_5px_rgba(0,0,0,0.3)] w-[30%] absolute top-[160px] right-12 pb-11">
          <div className="border rounded-lg overflow-hidden h-[100%]">
            <Carousel>
              <div className="h-full">
                <img
                  src={`${storageUrl}/course/${courseDetail.thumbnail}`}
                  alt=""
          ...
          <div className="mt-8 mx-5">
            <p>
              <UserOutlined /> Số lượng người đăng kí khóa học:{" "}
              <span className="text-blue-600">
                {courseDetail.totalPurchased === null
                  ? 0
                  : courseDetail.totalPurchased}
              </span>
            </p>
          </div>
          <div className="mt-3 mx-5">
            <p>
              <ClockCircleOutlined /> Tổng thời lượng:{" "}
              <span className="text-green-600">
                {countTotalTime(courseDetail)}
              </span>
            </p>
          </div>
          <div className="mt-3 mx-5 flex gap-1 items-center">
            <IoIosInfinite className="text-[20px] seft-center" />{" "}
            <p> Quyền truy cập đầy đủ suốt đời</p>
          </div>
          <div className="mt-3 mx-5">
            <p>
              <HistoryOutlined /> Cập nhật lần cuối:{" "}
              <span>
                {courseDetail.updatedAt === ""
                  ? dayjs(courseDetail.createdAt).format("DD/MM/YYYY")
                  : dayjs(courseDetail.updatedAt).format("DD/MM/YYYY")}
              </span>
            </p>
          </div>
